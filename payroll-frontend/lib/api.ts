...
class ApiClient {
    ...
    async getSalaryStructure(employeeId: number): Promise<SalaryStructure> {
        return this.request<SalaryStructure>(`/api/autopay-os/salary-structures/${employeeId}`);
    }

    async createSalaryStructure(data: SalaryStructureCreate): Promise<SalaryStructure> {
        return this.request<SalaryStructure>('/api/autopay-os/salary-structures', {
            method: 'POST',
            body: JSON.stringify(data),
        });
    }
    ...
    async processAutoPayOS(data: AutoPayOSProcessRequest): Promise<AutoPayOSRecord[]> {
        return this.request<AutoPayOSRecord[]>('/api/autopay-os/process', {
            method: 'POST',
            body: JSON.stringify(data),
        });
    }
    ...
    async getAutoPayOSSummaries(): Promise<AutoPayOSSummary[]> {
        return this.request<AutoPayOSSummary[]>('/api/autopay-os/summaries');
    }

    async getAutoPayOSHistory(employeeId: number): Promise<AutoPayOSRecord[]> {
        return this.request<AutoPayOSRecord[]>(`/api/autopay-os/history/${employeeId}`);
    }
